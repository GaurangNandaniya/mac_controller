...
def media_status():
    """Current output volume + mute, plus best-effort now-playing metadata."""
    try:
        status_data = driver.get_media_status()
        return jsonify({
            "status": "success",
            "volume": status_data.get("volume"),
            "muted": status_data.get("muted"),
            "nowPlaying": status_data.get("nowPlaying"),
        })
    except Exception as e:
