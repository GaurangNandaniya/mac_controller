...
import mss
from src.platform_driver import driver
...
def brightness_up():
    try:
        driver.adjust_display_brightness(up=True)
...
def brightness_down():
    try:
        driver.adjust_display_brightness(up=False)
...
def get_battery():
    try:
        percentage = driver.get_battery_percentage()
...
def set_keyboard_light(level):
    try:
        driver.set_keyboard_brightness(level)
        return jsonify({"status": "success"})
    except Exception as e:
...
def capture_and_lock():
    try:
        ...
        driver.capture_screen_and_webcam(session_path)

        logger.info("Capture and lock successful")
        return jsonify({"status": "success"})

    except Exception as e:
        logger.error(f"Error in capture_and_lock: {str(e)}")
        return jsonify({"status": "error", "error": str(e)}), 500
...
def press_key():
    """Press a key (optionally with modifiers) via the platform driver."""
    try:
        data = request.get_json(silent=True) or {}
        key = data.get("key")
        ...
        driver.press_special_key(key, modifiers)
        logger.info(f"pressKey: {modifiers}+{key}")
        return jsonify({"status": "success"})
    except ValueError as e:
        # Driver rejected the key (e.g. unknown key name) — a client error, not 500.
