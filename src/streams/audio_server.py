...
def audio_stream(ws):
    ...
    p = pyaudio.PyAudio()
    loopback_params = driver.get_loopback_pyaudio_params(p)
    
    if "input_device_index" not in loopback_params:
        logger.warning("System loopback audio device not found. Falling back to default input (Microphone).")
    ...
    try:
        ...
        open_kwargs.update(loopback_params)

        stream = p.open(**open_kwargs)
        logger.info(f"Audio WS Client Connected. Streaming Int16 with params {loopback_params}...")
