...
**Watch token (third token type, `type:'watch'`):** `auth_manager.generate_watch_token()` mints a scoped, optionally no-expiry token for Apple Shortcuts (mint via the root `generate_watch_token.py` script). It is validated **statelessly** — signature + optional `exp` only, NOT stored in `permanent_tokens`/`connected_devices` — so **"Revoke All Devices" does not affect it** (rotate `AUTH_SECRET_KEY` to invalidate). Scope is enforced by an allowlist (`is_watch_path_allowed()` in `auth_manager.py`): only `/media/*`, `/system/lock`, `/system/sleep`, `/system/capture-and-lock` (else 403). The middleware + `auth_required` apply the allowlist; `mouse_ws` rejects watch tokens outright. Token is signature-validated, so a freshly minted one works without server restart — but the **server must be running code that knows the `watch` type** (restart after deploying this feature).
...
## Last Updated
2026-06-16 — Added a scoped **Apple Watch token** (`type:'watch'`) for Apple Shortcuts: `auth_manager.generate_watch_token()` + root `generate_watch_token.py` script; stateless validation (survives Revoke-All); path allowlist `is_watch_path_allowed()` limits it to media + lock/sleep/capture-and-lock; `mouse_controller` rejects it. See Auth flow above.
